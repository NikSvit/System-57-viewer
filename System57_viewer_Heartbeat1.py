...
def changeState():
    ...
    mavlink_message = drone.recv_match(type = 'HEARTBEAT', blocking = True, timeout=0.01 )
    ...
    mavlink_message = drone.recv_match(type='GPS_RAW_INT', blocking=True, timeout=0.01)
    if mavlink_message is not None:
        ...
        GPSTimeLabel.configure(text = 'GPS time - '+time.strftime("%m/%d/%Y, %H:%M:%S",time.gmtime(mavlink_message.time_usec/1000000)))
        print (time.gmtime(mavlink_message.time_usec/1000000))
